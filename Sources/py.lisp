...
(defparameter *default-py-function-text*
  ...
  '(";;; edit a valid python code,"
    ...
    "(lambda () (format nil                \"PUT_YOUR_CODE_HERE_INSIDE_QUOTES\"              ))"))
...
(defmethod compile-patch ((self OMPYFunction))
  ...
  (handler-bind
    ...
    (let* ((lambda-expression (read-from-string (reduce #'(lambda (s1 s2) (concatenate 'string s1 (string #\Newline) s2)) (text self)) nil))
           (function-def
...
(defclass OMpyFunctionFile (OMPersistantObject OMpyFunction) ()
  (:default-initargs :icon :lisp-f-file)
  (:metaclass omstandardclass))

(add-om-doctype :textfun "olsp" "Text (py) Function")

(defmethod object-doctype ((self OMpyFunction)) :textfun)

(defmethod type-check ((type (eql :textfun)) obj)
  (let ((fun (ensure-type obj 'OMpyFunction)))
    (when fun
...
; For conversions
(defmethod internalized-type ((self OMpyFunctionFile)) 'OMpyFunctionInternal)
(defmethod externalized-type ((self OMpyFunction)) 'OMpyFunctionFile)
(defmethod externalized-icon ((self OMpyFunction)) :py-f-file)
...
" code))
      (save-python-code (om::save-as-text python-code (om::outfile "om-ckn-code.py")))
      (prepare-cmd-code (list->string-fun (list (namestring save-python-code)))))
      (om-cmd-line (string+ "python " prepare-cmd-code))
      (mp:process-run-function "del-py-code" () (lambda (x) (ckn-clear-the-file x)) (om::outfile "om-ckn-code.py"))
      'end))
...
" (py-inside-om code)))
      (save-python-code (om::save-as-text python-code (om::outfile "my-code.py")))
      (prepare-cmd-code (list->string-fun (list (namestring save-python-code)))))
      (ckn-cmd-line (string+ "python " prepare-cmd-code " > " (list->string-fun (list (namestring (outfile "python-data.txt"))))))
      (let* (
            (data (make-value-from-model 'textbuffer (outfile "python-data.txt") nil)))
            (mp:process-run-function "del-py-code" () (lambda (x) (ckn-clear-the-file x)) (om::outfile "my-code.py"))
            (mp:process-run-function "del-data-code" () (lambda (x) (ckn-clear-the-file x)) (om::outfile "python-data.txt"))
            (contents data))))
...
(mapcar (lambda (x) (read-from-string x)) result))
